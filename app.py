...
def calculate_labels_and_weight(device, qty, shipping_type):
    ...
    if shipping_type == "New Car":
        max_qty_per_box = (
            50 if device not in ["Arrow - device only", "EVO - device only"] else 100
        )
        ...
        if total_boxes:
            if total_boxes == 1:
                ...
                labels.append(
                    f"({total_boxes}) Labels @ {single_box_weight:.2f} lbs EACH"
                )
        ...
        if boxes_20:
            if boxes_20 == 1 and (remaining_10 + box_size_10) <= box_size_20:
                ...
                labels.append(
                    f"({boxes_20}) Labels @ {single_box_weight_20:.2f} lbs EACH"
                )

        if boxes_10 or remaining_10:
            if boxes_10 == 1 and remaining_10:
                labels.append(
                    f"(1) Label @ {(box_size_10 + remaining_10) * weight_per_device:.2f} lbs"
                )
            else:
                if boxes_10:
                    labels.append(
                        f"({boxes_10}) Labels @ {single_box_weight_10:.2f} lbs EACH"
                    )
                if remaining_10:
                    labels.append(
                        f"(1) Label @ {(remaining_10 * weight_per_device):.2f} lbs"
                    )
